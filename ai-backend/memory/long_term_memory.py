...
class LongTermMemoryStore:
    ...
    async def store_memory(self, user_id: str, content: str, trace_id: str = None) -> bool:
        ...
        if await self._is_duplicate(user_id, content):
            log_pipeline_event(event='memory_duplicate_skipped', trace_id=trace_id, metadata={
                'user_id': user_id, 'content_preview': content[:50]
            })
            return False
        ...
        await asyncio.to_thread(lambda: self.collection.add(
            ids=[memory_id],
            documents=[content],
            metadatas=[{
                "user_id": user_id,
        ...
        log_pipeline_event(event='memory_stored', trace_id=trace_id, metadata={
            'user_id': user_id, 'memory_id': memory_id, 'content_preview': content[:50]
        })
        return True

    async def retrieve_memories(self, user_id: str, query: str, top_k: int = 5, trace_id: str = None) -> list[dict]:
        """Retrieve top-K relevant memories for a user. Updates access metadata."""
        results = await asyncio.to_thread(lambda: self.collection.query(
            query_texts=[query],
        ...
        if update_ids:
            await asyncio.to_thread(lambda: self.collection.update(
                ids=update_ids,
                metadatas=update_metadatas
            ))
        log_pipeline_event(event='memory_retrieved', trace_id=trace_id, metadata={
            'user_id': user_id, 'count': len(memories),
    ...
    async def list_memories(self, user_id: str) -> list[dict]:
        ...
        results = await asyncio.to_thread(lambda: self.collection.get(
            where={"user_id": user_id},
    ...
    async def _is_duplicate(self, user_id: str, content: str) -> bool:
        ...
        try:
            ...
            log_pipeline_event(event='memory_duplicate_check_error', trace_id=trace_id, metadata={'error': str(exc)})
            return False
        return False
