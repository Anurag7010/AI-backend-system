...
function validate(values: FormState): FormErrors {
  ...
  else if (!/\d/.test(values.password)) errs.password = 'Password must contain at least one number'
  if (!values.confirmPassword) errs.confirmPassword = 'Please confirm your password'
  else if (values.confirmPassword !== values.password) errs.confirmPassword = 'Passwords do not match'
  return errs
}
...
function AuthInput({
  label, type, name, value, onChange, error, placeholder, autoComplete, icon,
  ...
  return (
    <div>
      <label className="text-xs font-medium text-parchment/80 mb-1.5 block">{label}</label>
      <div className="relative">
        <span className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-ash-gray">{icon}</span>
        <input
          type={type}
          name={name}
          value={value}
    ...
    <div className="flex min-h-screen bg-ember-black">
      ...
      <div className="flex flex-1 items-center justify-center px-6 py-12">
        ...
        >
          ...
          <h1 className="font-cormorant text-3xl font-light tracking-tight text-parchment">Forge Your Identity</h1>
          <p className="mt-1.5 text-sm text-ash-gray">
            Claim the fire. Become eternal.{' '}
            <Link href="/login" className="font-medium text-parchment/80 hover:text-parchment transition-colors">
              Sign in
            </Link>
          </p>
